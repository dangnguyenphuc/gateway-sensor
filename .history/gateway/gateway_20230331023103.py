    ...
    try:
        if received["id"] == feedID["FanValue"]:
            if received["last_value"] != "#":
                ...
                fan_collection.insert_one({"timestamp": f'{current_time}', "serverSide": True, "status": "ON", "value": int(fanValue)})
                cmd = "!FAN:ON:"+fanValue+"#"
        ...
        if received["id"] == feedID["LightValue"]:
            if received["last_value"] != "#":
                ...
                Bub_collection.insert_one({"timestamp": f'{current_time}', "serverSide": True, "status": "ON", "value": int(lightValue)})
        ...
        if received["id"] == feedID["button1"]:
            if received["last_value"] == "ON":
                ...
                fan_collection.insert_one({"timestamp": f'{current_time}', "serverSide": True, "status": "ON", "value": int(lightValue)})
...
def processData(data):
    ...
    if splitData[0] == "TE" and splitData[2] == "LI":
        ...
        sensor_collection.insert_one({"timestamp": f'{current_time}', "serverSide": False, "temp": int(splitData[1]), "light" : int(splitData[1])})
    ...
    if splitData[0] == "BUT":
        if splitData[1] == "1":
            ...
            if splitData[1] == "ON":
                ...
                fan_collection.insert_one({"timestamp": f'{current_time}', "serverSide": False, "status": "ON", "value": int(splitData[3])})
            else: 
                # client.publish(feed + AIO_FEED_PUBLISH[6], 0)
                fan_collection.insert_one({"timestamp": f'{current_time}', "serverSide": False, "status": "OFF", "value": 0})
        ...
        if splitData[2] == "2":
            ...
            if splitData[1] == "ON":
                ...
                Bub_collection.insert_one({"timestamp": f'{current_time}', "serverSide": False, "status": "ON", "value": int(splitData[3])})
            else: 
                # client.publish(feed + AIO_FEED_PUBLISH[7], 0)
                Bub_collection.insert_one({"timestamp": f'{current_time}', "serverSide": False, "status": "OFF", "value": 0})
