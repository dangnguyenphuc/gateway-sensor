...
def wind():
    ...
    frm = tk.Frame(window, width=500, height=300, background="#ffffff")
    frm.place(x=300, y=1000)
    canvas = FigureCanvasTkAgg(plt.gcf(), master=frm)
    canvas.get_tk_widget().grid(column=0, row=1)
    # Create two subplots in row 1 and column 1, 2
    plt.gcf().subplots(1, 2)
    
    ani = FuncAnimation(plt.gcf(), animate, interval=5000, blit=False)
    window.mainloop()
