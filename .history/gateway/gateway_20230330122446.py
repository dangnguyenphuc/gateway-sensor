...
client = MongoClient('mongodb+srv://dangnguyenblackie:dang1012@cluster0.mfwrjvy.mongodb.net/?retryWrites=true&w=majority')
    # database 
db = client["SmartHome"]
    # collection
...
sensor_collection = db["Temp_sensor"]
device_collection = db["Device"]

# res = user_collection.find().sort("id",-1).limit(1)
fan_collection.insert_one({"_id": 1, "timestamp": f'{current_time}', "status": "ON", "value" : 50})
